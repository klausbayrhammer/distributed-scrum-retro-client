...
describe('the firebase repository', () => {
  ...
  it('should be able to set and fetch the the initial app state', () => initializeRepository({ G: { title: 'Good' } }).then((repository) => {
    repository.columns.should.eql([
      { id: 'G', title: 'Good', createCard: undefined, cards: [] },
    ]);
  }));

  it('should be able to create cards', () => initializeRepository({ G: { title: 'Good' } }).then(repository => new Promise((resolve) => {
  ...
  it('should be able to vote for cards', (done) => {
  ...
  it('should be able to remove votes for cards', (done) => {
    initializeRepository({ G: { title: 'Good', cards: { card1: { title: 'title', votes: 0 } } } })
      .then((repository) => {
        repository.onChange((newRepository) => {
          newRepository.columns[0].cards[0].votes.should.equal(-1);
  ...
  it('should be able to delete cards', (done) => {
    initializeRepository({ G: { title: 'Good', cards: { card1: { title: 'title', votes: 0 } } } })
      .then((repository) => {
        repository.onChange((newRepository) => {
  ...
  it('should set the createCard flag for a column if the prepareCreateCard function is invoked', () => initializeRepository({ G: { title: 'Good' } })
